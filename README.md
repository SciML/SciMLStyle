...
begin with, an grow the generic support over time. All recommended functionality should be tested, any known
genericity issues should be documented in an issue (and with a `@test_broken` test when possible). However, a
...
If `f(A)` takes the input of some collections and computes and output from those collections, then it should be
...
### Macros should limited and only be used for syntactic sugar
...
[`@muladd`](https://github.com/SciML/MuladdMacro.jl) is trivial to picture on a code (it recrusively
...
with independent versioning and package mangagement. An example of this is seen in 
...
- Exported variables should be considered as part of the the public API, and changing their interface consistutes a
  breaking change. 
...
- Test scripts should be grouped based on categories, for example tests of the interface vs vs tests for numerical
...
- A `GROUP` environment variable should be used to specify test groups for parallel testing in continuous integration.
  A fallback group `All` should be used to specify all of the tests that should be ran when a developer runs `]test Package`
...
- Tests should include downstream tests to major packages which use the functionality, to ensure continued support.
  ...
  support was broken (preferrably in the form of a PR which fixes support), and the package should be given a major
...
is preferred. If genericity is required, then parametric typing is preferred, i.e.:
